...
{
	CreateCustomHindiTextBox(elementId,"",100,40,false);
}
function CreateCustomHindiTextBox(elementId,defaultValue,col,row,showHelp)
{
	hinTextBoxObj= new TextBoxInput();
	hinTextBoxObj.Id=elementId;
	hinTextBoxObj.Name=elementId;
	hinTextBoxObj.Value=defaultValue;
	hinTextBoxObj.cols=col;
	hinTextBoxObj.rows=row;
...
document.write("<textarea");
	if(textBoxParam.Value!=null)
	{
		document.write(" value='"+textBoxParam.Value+"'"); 
                //g.value='"+textBoxParam.Value+"';	
} 
	if(textBoxParam.rows!=null)
	{
		document.write(" rows='"+textBoxParam.rows+"'"); 
          //     g.rows='"+textBoxParam.rows+"';	
}
if(textBoxParam.cols!=null)
	{
		document.write(" cols='"+textBoxParam.cols+"'"); 
   	//g.cols='"+textBoxParam.cols+"';
}
	if(textBoxParam.Class!=null)
	{
		document.write(" Class='"+textBoxParam.Class+"'"); 
  //g.Class='"+textBoxParam.Class+"';	
}
	if(textBoxParam.Name!=null)
	{
		document.write(" Name='"+textBoxParam.Name+"'"); 
 //g.Name='"+textBoxParam.Name+"';	
}

	document.write(" id='"+textBoxParam.Id+"'");
	//g.id='"+textBoxParam.Id+"';
	if(textBoxParam.UseHinkhoj)
	{
        ...
        /*g.onkeypress='if (!inputBoxIds["+inputBoxCount+"].isEng){ return processInput(event,inputBoxIds["+inputBoxCount+"]);} else {return true;}';
        g.onKeyUp='changeCursor(this);';
        
        g.onkeydown='positionChange(event,inputBoxIds["+inputBoxCount+"]);';
        g.onFocus='changeCursor(this);';
        g.onClick='changeCursor(this);';*/
	}
	else
	{
		document.write(" onmouseout=ChangeHindi('"+textBoxParam.Id+"') ");
		g.onmouseout=ChangeHindi('"+textBoxParam.Id+"');
	}
	
	//f.appendChild();
        //document.getElementBytagName('body').[0].appendChild(f);
        document.write("form='entercode'");	
	document.write(">");
	documnet.write("</tetxarea>");
        document.write("</body>");
        document.write("</html>");
        //document.write("<br>");
//document.write("<input");
//document.write("type='submit'");
//document.write("value='RUN'");
//document.write("/>");
}
function CreateHindiTextArea(elementId)
{
	CreateCustomHindiTextArea(elementId,"",700,450,false);
}
